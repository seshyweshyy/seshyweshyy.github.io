...
class Game:
    ...
    def handle_collisions(self):
        ...
        for enemy in self.enemies:
            if enemy.position.distance_to(self.player.position) <= ENEMY_RADIUS + PLAYER_RADIUS:
                ...
                enemy.hit = True
        self.enemies = [enemy for enemy in self.enemies if not enemy.hit]
    ...
    def draw(self):
        ...
        if self.game_over:
            ...
            subtitle = self.font.render("Press R to restart", True, (200, 200, 200))
...
async def main():
    ...
    while game.running:
        delta = game.clock.tick(FPS) / 1000.0
        game.handle_events()
